...
export class BrowserManager {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          this.logger.debug(`Could not copy ${entry.name}: ${String(_error)}`);
        }
      }
    }
  }
  ...
  async initialize(): Promise<BrowserContext> {
    ...
    try {
      ...
      this.logger.error(`Failed to initialize browser: ${(_error instanceof Error ? _error.message : String(_error))}`);
      throw _error;
    }
  }
  ...
  async saveStorageState(): Promise<void> {
    if (this.context) {
      ...
      try {
        ...
        this.logger.debug(`Could not save storage state (persistent context handles this automatically): ${(_error instanceof Error ? _error.message : String(_error))}`);
      }
    }
  }
